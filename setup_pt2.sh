...
######################################
#######___________IPXE_________#######
######################################
# See notes at bottom of file

# set tag for requests from ipxe
dhcp-match=set:ipxe,175 # iPXE sends a 175 option.

# load ipxe.pxe (or undionly.kpxe) for clients not tagged with 'ipxe'.
dhcp-boot=tag:!ipxe,$DST_IPXE/ipxe.pxe

# load our menu (We are going to create this menu automatically)
dhcp-boot=$DST_IPXE/menu.ipxe

### Disable Router and DNS (If you have a router already that you want giving out IPs)
...
# Boot for iPXE. The idea is to send two different filenames,
# the first loads iPXE, and the second tells iPXE what to
# load. The dhcp-match sets the ipxe tag for requests from iPXE.

# You could replace our menu system with Robin's bootstrap method, but that defeats
# the purpose of what we're doing in the menu builder sections.
# When ipxe.pxe/undionly.kpxe issues it's own DHCP request we could serve bootstrap.ipxe over http
# using Robin Smidsrød's bootstrap method provided at https://gist.github.com/2234639
# dhcp-boot=http://$IP_ETH0/bootstrap.ipxe
...
[www]
    path = /var/www/html
    comment = www folder for ipxe menu
    guest ok = yes
    guest only = yes
    browseable = yes
    read only = no
...
#### IPXE ADD-IN
handle_ipxe_menu() {
    echo 'Setting up the IPXE Menu is not a working function yet';
    exit 0;

    # $1 : menu short name
    # $2 : menu file name
    ######################################################################
    local FILE_MENU=$DST_TFTP_ETH0/$1/menu.ipxe/$2
    local FILE_BASE=http://$IP_ETH0/srv
    ######################################################################
    ## INFO:
    ## The entry before -- means that it will be used by the live system / the installer
    ...
    echo -e "\e[32mhandle_pxe_menu(\e[0m$1\e[32m)\e[0m";
    echo -e "\e[36m    setup sys menu for pxe\e[0m";
    if ! [ -d "$DST_TFTP_ETH0/$1/menu.ipxe" ]; then sudo mkdir -p $DST_TFTP_ETH0/$1/menu.ipxe; fi
    if [ -d "$DST_TFTP_ETH0/$1/menu.ipxe" ]; then
        sudo sh -c "cat << EOF  > $FILE_MENU
#!ipxe
...
. "$script_dir/p2-include-ipxe-menu.sh"
}


handle_ipxe() {
    echo 'IPXE files installed to:' $DST_TFTP_ETH0'/'$DST_IPXE;
    [[ -d "$DST_TFTP_ETH0/$DST_IPXE" ]] || sudo mkdir -p $DST_TFTP_ETH0/$DST_IPXE/;
    [[ -f "$DST_TFTP_ETH0/$DST_IPXE/undionly.kpxe" ]] || sudo wget -O $DST_TFTP_ETH0/$DST_IPXE/undionly.kpxe http://boot.ipxe.org/undionly.kpxe;
    [[ -f "$DST_TFTP_ETH0/$DST_IPXE/ipxe.pxe" ]] || sudo wget -O $DST_TFTP_ETH0/$DST_IPXE/ipxe.pxe http://boot.ipxe.org/ipxe.pxe;
    [[ -f "$DST_TFTP_ETH0/$DST_IPXE/ipxe.efi" ]] || sudo wget -O $DST_TFTP_ETH0/$DST_IPXE/ipxe.efi http://boot.ipxe.org/ipxe.efi;
...
handle_iso() {
    ...
    while [ $# -gt 0 ]; do
	    ...
	    is_local)
		is_local=true;
                echo -e "\e[36m    local file. using rsync instead of wget\e[0m";
		;;
            *)
                fstab_options="$1"
    ...
    if [ -z $URL ]; then
	   ...
	   [[ "$is_local" ]] && sudo rsync -xa --info=progress2 $URL $DST_ISO/$FILE_ISO || sudo wget -O $DST_ISO/$FILE_ISO  $URL;
...
if ( [[ $# -gt 0 ]] ) then 
    case $1 in
        m)echo -e "\e[36m    menu rebuild-only mode\e[0m"; 
            handle_pxe; 
            exit 0;
        ;;
        i)echo -e "\e[36m    ipxe mode enabled\e[0m"; 
            handle_ipxe;
        ;;
    esac
fi
